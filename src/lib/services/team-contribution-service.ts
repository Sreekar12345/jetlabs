...
export async function getTeamContributionData(
  ...
  const dependencyRiskTeams = executionTeams.filter((team) => team.dependencyScore >= 44);
  ...
  const overloadedLeadCount = executionTeams.reduce(
    (sum, team) =>
      sum + team.members.filter((member) => member.aiLabel === "Burnout Risk").length,
    0,
  );
  const averageTrust = average(
    executionTeams.flatMap((team) =>
      team.members.map((member) =>
        average(member.trustSignals.map((signal) => signal.value)),
      ),
    ),
  );
  ...
  return {
    stats: [
      {
        ...
        value: `${executionTeams.filter((team) => team.riskState === "Healthy").length}/${executionTeams.length}`,
    ...
    alerts: executionTeams.slice(0, 6).map((team) =>
      buildActivityItem({
        id: team.id,
