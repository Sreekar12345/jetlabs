...
function buildContributionHeatmap(team: HealthTeamRecord, inactiveDays: number) {
  ...
  const memberBands = team.students.map((member: any) =>
...
function getActiveMilestone(team: HealthTeamRecord) {
  return (
    team.project.milestones.find((milestone: any) => milestone.status === "BLOCKED") ??
    team.project.milestones.find((milestone: any) => milestone.status === "IN_PROGRESS") ??
    team.project.milestones.find((milestone: any) => milestone.status === "PENDING") ??
...
function buildDetectedSignals(snapshot: TeamHealthSnapshot, riskLayer: ProjectHealthTeam["riskLayer"]) {
  ...
  const maxContribution = Math.max(...team.students.map((member: any) => member.contributionScore), 0);
...
function buildQualitySignals(snapshot: TeamHealthSnapshot) {
  ...
  const contributionSpread = team.students.length
    ? Math.min(
        100,
        100 -
          (Math.max(...team.students.map((member: any) => member.contributionScore)) -
            Math.min(...team.students.map((member: any) => member.contributionScore))),
      )
    : 0;
...
function buildExecutionTimeline(team: HealthTeamRecord) {
  return team.project.analytics.slice(-5).map((snapshot: any) => ({
...
function buildTeamHealthSnapshot(team: HealthTeamRecord): TeamHealthSnapshot {
  ...
  const averageContribution = average(
    team.students.map((member: any) => member.contributionScore),
  );
